...
_CM_DESCRIPTIONS: dict[IssueType, str] = {
    ...
    IssueType.HIGH_REGISTER_PRESSURE: "Large/numerous vector<>/matrix<> live ranges strain the GRF — either shrink the per-thread tile / split the loop to use fewer registers, OR keep the big tile and select the LARGE register file with a build directive at the top of the source: `// xe-forge-build: -Qxcm_register_file_size=256` (the harness honors it). Use the large-GRF directive when register pressure is high even BEFORE an actual spill; it doubles per-thread registers but halves resident threads (occupancy), so which way wins is hardware-dependent — measure both",
